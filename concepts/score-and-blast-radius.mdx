...
Each scan writes a JSON log and, when the environment changed, a lockfile-shaped snapshot to your user cache directory. On later runs the score box shows the delta and the report ends with the actions that would move the score most. Real output:

```
  │  ↓ -21 pts since last scan (was 39/100)                     │

  Top actions to improve your score
  → Fix Dangerous capability: arbitrary filesystem write across 2 servers  (~+82 pts)
  → Fix Known vulnerable package across 3 servers  (~+82 pts)
  → Fix Server can write agent-trusted state across 3 servers  (~+60 pts)
```
