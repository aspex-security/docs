...
`--share` writes a privacy-safe Markdown summary with server commands, env values and paths redacted. `--report soc2` or `--report iso27001` maps findings to controls and emits PASS/FAIL per control. `--html` writes a self-contained report.
