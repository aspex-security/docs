...
$ aspex
  ...
  ▸ 2 calls tripped a detection rule. Most common: cross-server data chain.
...
$ aspex scan
  ...
  Blast radius HIGH  ✓ reads credentials · ✓ arbitrary egress · ✓ command execution

  CRITICAL  AP001  Potential sensitive data exfiltration path  confidence: high
     filesystem   └─ allowed root /Users/steven (home: ~/.ssh, ~/.aws, browser profiles)
     playwright   └─ browser_navigate: reaches network destinations
```
...
$ aspex explain "Can this agent exfiltrate SSH keys?"
  ...
  YES  a plausible path exists
     ✓ external content enters the agent's context      brave-search, playwright
     ✓ a file read reaches ~/.ssh                        filesystem (root /Users/steven)
     ✓ network egress to any destination                 playwright.browser_navigate
  Confidence: HIGH
```
